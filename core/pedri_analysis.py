...
def is_pedri_event(ev):
    return safe_get(ev, ("player", "id")) == PEDRI_ID


def is_pass(ev):
    return safe_get(ev, ("type", "name")) == "Pass"


def is_completed_pass(ev):
    return is_pass(ev) and safe_get(ev, ("pass", "outcome", "name")) in (
        None,
        "Complete",
    )
...
def is_shot(ev):
    return safe_get(ev, ("type", "name")) == "Shot"


def is_dribble(ev):
    return safe_get(ev, ("type", "name")) == "Dribble"


def dribble_outcome(ev):
    return safe_get(ev, ("dribble", "outcome", "name"))


def is_carry(ev):
    return safe_get(ev, ("type", "name")) == "Carry"


def is_pressure(ev):
    return safe_get(ev, ("type", "name")) == "Pressure"
...
def is_interception(ev):
    return safe_get(ev, ("type", "name")) == "Interception"


def is_ball_recovery(ev):
    return safe_get(ev, ("type", "name")) == "Ball Recovery"
...
def get_end_loc(ev):
        ...
        if is_pass(ev)
        else ev.get("carry", {}).get("end_location") if is_carry(ev) else None
    )
...
def minutes_played_estimate(events):
    ...
    for ev in events:
        if safe_get(ev, ("type", "name")) == "Substitution":
            ...
            if in_player == PEDRI_ID:
                off_min = minute
            if out_player == PEDRI_ID:
                on_min = minute
...
def plot_heatmap_locations(events, title, outname, gridsize=30, mincnt=1):
    locs = np.array(
        [get_loc(e) for e in events if is_pedri_event(e) and get_loc(e) is not None],
        dtype=float,
    )
    if locs.size == 0:
        print("⚠️  No locations for heatmap.")
        return
    x, y = locs[:, 0], locs[:, 1]
...
def generate_pedri_radars(df: pd.DataFrame):
    ...
    _radar_plot(
        np.array(pct_vals, dtype=float),
        axis_labels,
        "Pedri – Percentile per 90 (vs own matches)",
        "pedri_radar_p90_percentile.png",
        rmax=100.0,
        rlabel="percentile",
    )
...
def _collect_pedri_pass_segments(events):
    ...
    return (
        ...
        np.array(xe),
        np.array(ye),
        np.array(prog_flags, dtype=bool),
    )
...
def generate_pass_maps(pedri_events_all):
    ...
    _plot_pass_map(
        ...
        xe,
        ye,
        "Pedri – Pass Map (all passes)",
        "pedri_pass_map_all.png",
        alpha=0.5,
    )
    # Progressive-only
    mask = prog
    _plot_pass_map(
        ...
        xe[mask],
        ye[mask],
        "Pedri – Pass Map (progressive only)",
        "pedri_pass_map_progressive.png",
        alpha=0.8,
    )
...
def main():
    ...
    if not per_match:
        print("❌ No Pedri events found across files.")
        return
    ...
    df.to_csv(extended_path, index=False, encoding="utf-8")
    print(f"💾 Saved: {extended_path}")

    compat_path = os.path.join(CSV_DIR, "pedri_per_match_stats.csv")
    df.to_csv(compat_path, index=False, encoding="utf-8")
    ...
    pd.DataFrame([agg]).to_csv(summary_path, index=False, encoding="utf-8")
    print(f"💾 Saved: {summary_path}")

    # ---- Charts (Top-20 where relevant) ----
    plot_bar(
        df.set_index("match_id")["key_passes"],
        "Key passes per match (Top 20)",
        "Match ID",
        "pedri_key_passes_per_match.png",
        top_n=TOP_N_DEFAULT,
    )
    plot_bar(
        df.set_index("match_id")["progressive_passes"],
        "Progressive passes per match (Top 20)",
        "Match ID",
        "pedri_prog_passes_per_match.png",
        top_n=TOP_N_DEFAULT,
    )
    plot_bar(
        df.set_index("match_id")["pass_pct"],
        "Pass completion % per match (Top 20)",
        "Match ID",
        "pedri_pass_pct_per_match.png",
        top_n=TOP_N_DEFAULT,
    )

    plot_hist(
        df["pressures_p90"].values,
        "Pressures per 90 (hist)",
        "pressures_p90",
        "pedri_pressures_p90_hist.png",
    )
    plot_hist(
        df["tackles_p90"].values,
        "Tackles per 90 (hist)",
        "tackles_p90",
        "pedri_tackles_p90_hist.png",
    )
    plot_hist(
        df["interceptions_p90"].values,
        "Interceptions per 90 (hist)",
        "interceptions_p90",
        "pedri_interceptions_p90_hist.png",
    )

    plot_heatmap_locations(
        all_pedri_events,
        "Pedri event density (all matches)",
        "pedri_event_heatmap_hexbin.png",
    )
